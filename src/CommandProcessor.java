...
public class CommandProcessor {
    ...
    public void processor(String line) {
        ...
        if (command.equals("insert")) {
            String parName = arr[1];
            int parX = Integer.valueOf(arr[2]);
            int parY = Integer.valueOf(arr[3]);

            Coordinates c = new Coordinates(parX, parY, parName);
            KVPair<String, Coordinates> pair = new KVPair<>(parName, c);
            data.insert(pair);
...
// number of white space delimited strings in the line
            ...
            if (numParam == 1) {
                ...
                int parX = Integer.valueOf(arr[1]);
                int parY = Integer.valueOf(arr[2]);

                data.remove(parX, parY);
            ...
            int parX = Integer.valueOf(arr[1]);
            int parY = Integer.valueOf(arr[2]);
            int parW = Integer.valueOf(arr[3]);
            int parH = Integer.valueOf(arr[4]);

            data.regionsearch(parX, parY, parW, parH);

        }
        else if (command.equals("duplicates")) {
            // calls the duplicates method, no parameters to be passed
            // (see the duplicates JavaDoc in the Database class for more
            // information)
            data.duplicates();
